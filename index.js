...
function getFullNames(runners) {
  ...
  runners.forEach(function (item) {
    let runnerNames = item.last_name + ", " + item.first_name;
    names.push(runnerNames);
  })
  return names;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnerShirt = runners.filter((shirt) => {
...
function tallyUpDonations(runners) {
  let tally = runners.reduce((accumulator, currentValue) => {
